...
pub async fn get_caller_address() -> Result<String, Error> {
    match get_key("SIWE".to_string()) {
        Ok(principal_as_str) => {
            match Principal::from_text(&principal_as_str) {
                Ok(principal) => {
                    let ic_siwe_provider = IcSiweProvider(principal);
                    let address = ic_siwe_provider
                        .get_address(ByteBuf::from(ic_cdk::caller().as_slice())).await;
                    match address {
                        Ok((response,)) => {
                            match response {
                        ...
                        Err(_) => {
                            Err(Error::Validation {
                                msg: "Error on calling siwe canister".to_string()
                            })
                        }
                    }
                },
